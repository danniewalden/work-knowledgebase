---
source_url: https://event-driven.io/en/vertical-slices-and-dependencies/
...
2026-08-10 — oskar dudycz — Software Architecture
...
I've written before about [how to slice the codebase effectively](https://event-driven.io/en/how_to_slice_the_codebase_effectively/) and shown [a worked module](https://event-driven.io/en/vertical_slices_in_practice/), and I listed "you can't share code between slices" as [one of the myths that grew around the pattern](https://www.architecture-weekly.com/p/my-thoughts-on-vertical-slices-cqrs). What I didn't do was show the positive answer. Minimise isn't zero, so what does the non-zero look like once you type it out?
...
[Structural typing](https://event-driven.io/en/structural_typing_in_type_script/) gives us that in TypeScript, and I think it's underused by people arriving from C# and Java, where the habit is to define a nominal interface and hand it around.
...
Both files go in `confirming-order/`, side by side. I showed this pattern in [Vertical Slices in practice](https://event-driven.io/en/vertical_slices_in_practice/) using an API endpoint and an external booking event that share a single command handler. With dependencies declared per handler, the event-triggered one looks like this:
...
And your first grouping will still be wrong somewhere. Mine usually is. That's fine, as long as being wrong stays cheap, which is the argument for [removability over maintainability](https://event-driven.io/en/removability_over_maintainability/) and for keeping the couplings visible rather than tucked away. A slice that states its dependencies is one you can move.
...
*p.s. If you're dealing with such issues, I'm happy to help you through consulting, training or mentoring.*
