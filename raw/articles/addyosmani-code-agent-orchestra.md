...
author: Addy Osmani
publication: AddyOsmani.com
published: 2026-03-26
retrieved: 2026-09-04
type: article
capture_note: >-
  IMPRESSION NOT MEASUREMENT for most of the quantities in this piece: "Parallelism (3x
  throughput)", "3-5 teammates is the sweet spot", "three focused agents consistently
  outperform one generalist agent working three times as long", "substantially cuts
  stuck agents", the per-agent token budgets (180k/280k) and the "roughly 220k tokens
  total" demo cost are the author's practitioner judgement and demo observations, not
  study results. The one exception is the AGENTS.md claim, which the author presents as
  research with figures — "LLM-generated AGENTS.md files offer no benefit and can
  marginally reduce success rates (~3%) while increasing inference costs by 20%+" and
  "~4% improvement" for developer-written files — but the underlying study is NOT NAMED
  OR LINKED anywhere in the page text, so the citation cannot be followed from this
  capture and the figures should not be treated as verified. NOT INDEPENDENT for the
  agentic-engineering framing generally: the author is a Director at Google Cloud AI and
  the piece closes by promoting his own O'Reilly book on the subject, and it links his
  own earlier posts ("The Future of Agentic Coding", "The Factory Model") as support for
  its own concepts. Full text of the article body captured (this is the author's
  write-up of an O'Reilly AI CodeCon talk); the four embedded demo videos are not
  reproducible in text and only their surrounding prose is present. Images are noted
  inline as [image: ...].
---
...
[image: conductor.webp - Conductor dashboard showing multiple Claude Code agents running in parallel with diff review]
...
[image: claude-code-web2.png - Claude Code Web interface showing parallel cloud tasks with GitHub integration]
...
[image: copilot-agents.png - GitHub Copilot Coding Agent showing the Agents panel and PR workflow]
...
[image: jules.webp - Jules cloud coding agent showing plan approval workflow and PR creation]
...
[image: codex.webp - OpenAI Codex Web interface showing parallel cloud tasks with verifiable evidence]
...
[image: cursor-glass.jpg - Cursor Glass interface showing agent management as the primary surface]
...
[image: vibe-kanban.avif - Vibe Kanban board showing parallel agent tasks with diff review]
...
This is the big one. Research has shown that "LLM-generated AGENTS.md files offer no benefit and can marginally reduce success rates (~3%) while increasing inference costs by 20%+." Developer-written context files, by contrast, provide a modest ~4% improvement. Never let an agent write to `AGENTS.md` directly. The lead must approve every line. Keep it shorter with clear sections:
...
[image: beyond.webp - Beyond Vibe Coding book cover]
