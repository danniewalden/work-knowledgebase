...
type: article
capture_note: >-
  OUT-OF-WINDOW BACKFILL (published 2026-03-04, ~5 months before the 14-day
  lookback). Captured deliberately by the 2026-08-15 watch as a gap-filling
  antecedent primary: it names the out-the-loop / in-the-loop / ON-the-loop
  taxonomy and "the agentic flywheel" four months before Osmani's
  "Own the Outer Loop" (2026-07-15) and LangChain's hill-climbing loop, both of
  which the wiki already holds. Flag for Dannie's accept/reject on ingest.
---

# Humans and Agents in Software Engineering Loops

**Kief Morris** — Kief Morris lives in London and works as a global cloud technology specialist for Thoughtworks.
...
*Figure 1: The why loop iterates over ideas and software, the how loop iterates on building the software* [image: the software delivery feedback loops — an upper "why" loop connected to a lower "how" loop; the why loop iterates over an idea and working software, the how loop iterates over interim artefacts like specs, code, and tests]
...
*Figure 2: The how loop has multiple levels of inner loops that work on smaller increments of the full implementation* [image: multiple levels of "how" loops supporting the "why" loop — an outer loop iterates on a feature, a middle loop iterates on stories, an inner loop iterates on code]
...
*Figure 3: Human runs the why loop, agent runs the how loop.* [image: an upper "why" loop with a human on top, connected to a lower "how" loop by a robot]
...
*Figure 4: Human runs the why loop and the how loop* [image: a single "why+how" loop with a human at the top and a robot at the bottom]
...
*Figure 5: Human defines the how loop and the agent runs it* [image: an upper "why" loop connected to a lower "how" loop by a human; a robot sits at the bottom of the "how" loop]
...
*Figure 6: Human directs agent to build and improve the how loop* [image: an upper "why" loop connected to a lower "how" loop by a human and a robot]
...
1. These days "ralph loop" is often used colloquially to mean just firing up a bunch of agents and leaving them to keep looping until (hopefully) they finish their task. But [as originally described](https://ghuntley.com/ralph/) the operator plays an important role in steering agents as they ralph. ↩
