...
author: Pramod Sadalage and Prem Chandrasekaran (Thoughtworks)
...
1. Data Contracts and Quality makes data *Trusted*. We start here, because a single wrong fact poisons every layer built on top of it.
2. Traceability and Governance records why an agent acted and bounds what it can reach, making data *Traceable* and *Governed*.
3. The context layer encodes what your metrics and entities mean, making data *Contextual*.
4. From Searchable to Actionable lets agents query live systems and write back, making data *Operational*.
...
None of this vocabulary is settled, and the words trip people up. The classic [semantic layer](https://en.wikipedia.org/wiki/Semantic_layer) (Business Objects, then LookML and Cube) bundled entities and relationships in with the metrics, so plenty of people still use "semantic layer" for the whole thing. We draw sharper lines because agents make the distinctions matter. We also say "model" rather than "layer" for the parts, because each is a body of definitions rather than a tier of infrastructure, and because that is what the tools call them: dbt declares `semantic_models`, and the vendor-neutral specification that came out of the Open Semantic Interchange initiative, now [Apache Ossie](https://ossie.apache.org/), is a semantic model specification.

The domain model goes by other names. Store its entities and relationships as a graph and you have a knowledge graph. Domain-driven design calls it a [domain model](https://martinfowler.com/eaaCatalog/domainModel.html), and its [Bounded Context](https://martinfowler.com/bliki/BoundedContext.html) is the reminder that no single model covers the whole enterprise. Chasing one [canonical model](https://martinfowler.com/bliki/MultipleCanonicalModels.html) is usually a mirage; each domain has its own, governed the federated way [Data Mesh](https://martinfowler.com/articles/data-mesh-principles.html#FederatedComputationalGovernance) describes.

The market name is **ontology**, and it is the same artefact. It has a longer pedigree than the current wave suggests: a [formal, explicit specification of a shared conceptualization](https://www.sciencedirect.com/science/article/abs/pii/S0169023X97000566), with RDF, OWL, and SHACL as the formal machinery. Strictly, an ontology describes and does not act. Two shipping products stretch it further. [Palantir's Foundry Ontology](https://www.palantir.com/docs/foundry/ontology/core-concepts) pairs its objects, properties, and links with the actions an agent may take on them, and [Databricks' Genie Ontology](https://www.databricks.com/blog/introducing-genie-one-genie-ontology-and-genie-agents) puts a living graph of a company's concepts on top of its governed metric definitions. The one real difference between Palantir's shape and ours is that it bundles the actions in. We keep them separate, because the write path carries different risk from the read path and benefits from being governed on its own terms.
...
*Capture note (watch, 2026-08-29): martinfowler.com is a tracked channel via the Böckeler/Fowler entry in the people watch (`people_scope: anything-substantive`). Off the Event Modeling method thread, but squarely on the KB's capability/context substrate: the article's "capability model" (permissions, owner, preconditions, reversibility class) is a data-architecture restatement of the [[business-capabilities]] boundary and reads as a sibling of [[devadoss-cead-capability-aligned-agent-design]]'s Agent Capability Contract; "agentic lineage" (traces/spans that record why) is the same argument as [[axoniq-ai-agent-explainability-why-infrastructure-needs-to-remember]] and [[fritzsche-why-your-software-cannot-explain-business-decisions]] made from the data side rather than the event-store side; "retrieved text informs, it never gates" is a design-level answer to [[willison-lethal-trifecta]]. Note the explicit disclosure that AI assistance was used in writing. Figures noted inline rather than embedded.*
