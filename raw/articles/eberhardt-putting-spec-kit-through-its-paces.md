...
[Artificial Intelligence] · 26 November 2025 · 12 min read · Colin Eberhardt
...
*Traditional development treats code as the source of truth, causing specifications to drift, creating a gap. Spec-Driven Development reverses this by making the specification the primary artifact that from which you can generate implementation plans and code. Specifications become precise enough for AI systems to produce consistent, working software directly from them. Maintaining or extending a system means updating the specification and regenerating code rather than modifying code manually. Debugging and refactoring involve clarifying the specification, allowing teams to focus on design clarity and problem-solving.*
...
*[Image: kartlog.png — the app's circuit-management screen.]*
...
*[Image: circuits-complete.png — the rebuilt circuit-management feature.]*
...
*[Image: timeline-sdd.png — a timeline of the spec-driven run, dominated by markdown generation and review.]*
...
*[Image: timeline-regular.png — the same timeline for the iterative-prompting run, markedly shorter.]*
...
*Capture note (2026-08-31, SDD-primaries hunt): this is THE Scott Logic SpecKit trial that [[ng-spec-driven-development-is-waterfall-in-markdown]] cites secondhand as "~10x slower" — the wiki's most load-bearing single-source dependency, open since 2026-08-16, now grounded in the primary. Author is **Colin Eberhardt, CTO of Scott Logic** (the log had recorded this only as "Scott Logic's SpecKit trial", with no author named). Note the date: **2025-11-26**, nine months before Ng's essay, so Ng is the downstream restatement, not the origin. Site is headless-fetchable. Content is CC BY-NC-SA 4.0. Nav/footer/related-posts/author-bio stripped; five images noted inline; commit hyperlinks preserved where they carry evidence, de-linked where they were bare "see this commit" references.*
